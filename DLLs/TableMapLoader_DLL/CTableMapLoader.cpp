...
void CTableMapLoader::ExtractConnectionDataFromCurrentTablemap(CTablemap *cmap) {
	...
	BasicScraper()->TablemapAccess()->GetClientSize("clientsizemin", &tablemap_connection_data[_number_of_tablemaps_loaded].ClientSizeMinX, &tablemap_connection_data[_number_of_tablemaps_loaded].ClientSizeMinY);
	BasicScraper()->TablemapAccess()->GetClientSize("clientsizemax", &tablemap_connection_data[_number_of_tablemaps_loaded].ClientSizeMaxX, &tablemap_connection_data[_number_of_tablemaps_loaded].ClientSizeMaxY);
	// Extract title text information
	BasicScraper()->TablemapAccess()->SetTitleText("titletext", tablemap_connection_data[_number_of_tablemaps_loaded].TitleText);
	// Extract negative title texs
	BasicScraper()->TablemapAccess()->SetTitleText("!titletext", tablemap_connection_data[_number_of_tablemaps_loaded].NegativeTitleText);
	CString s = "";
	for (int i=0; i<k_max_number_of_titletexts; i++) {
		s.Format("titletext%d", i);
		BasicScraper()->TablemapAccess()->SetTitleText(s, tablemap_connection_data[_number_of_tablemaps_loaded].TitleText_0_9[i]);
		s.Format("!titletext%d", i);
		BasicScraper()->TablemapAccess()->SetTitleText(s, tablemap_connection_data[_number_of_tablemaps_loaded].NegativeTitleText_0_9[i]);		
	}
	...
	for (int i=0; i<k_max_number_of_tablepoints; ++i) {
		...
		if (BasicScraper()->TablemapAccess()->GetTableMapRegion(s, &tablemap_connection_data[_number_of_tablemaps_loaded].TablePoint[i])) {
		  ++tpcount;
		} else {
      break;
    }
