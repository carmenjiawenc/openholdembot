		...
		CDialog(CDlgFormulaScintilla::IDD, pParent), m_winMgr(ScintillaFormulaMap) 
{
  ...
  if (Formula()->FunctionCollection()->IsOpenPPLProfile()) {
...
void CDlgFormulaScintilla::PopulateFormulaTree() {
  ...
  p_OH_script_object = Formula()->FunctionCollection()->GetFirst();
  while (p_OH_script_object != NULL) {
    ...
    p_OH_script_object = Formula()->FunctionCollection()->GetNext();
  }
...
void CDlgFormulaScintilla::OnTvnSelchangedFormulaTree(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  if ((m_FormulaTree.GetParentItem(m_FormulaTree.GetSelectedItem()) == NULL && s != "notes") 
	...
	if (COHScriptObject::IsNotes(s)
      ...
      COHScriptObject *p_function_or_list = Formula()->FunctionCollection()->LookUp(s);
...
void CDlgFormulaScintilla::OnNew() {
  ...
  if (newdlg.CSnewname == "f$call") {
    ...
    Formula()->FunctionCollection()->LookUp("f$call")->SetText("  ");
    PopulateFormulaTree();
    return;
  }
  if (Formula()->FunctionCollection()->Exists(newdlg.CSnewname)) {
  ...
  if (newdlg.is_function == false) {
    ...
    Formula()->FunctionCollection()->Add((COHScriptObject*)p_new_list);
    // Add to tree
...
void CDlgFormulaScintilla::OnRename() {
  ...
  if (!Formula()->FunctionCollection()->Rename(rendlg.CSoldname, rendlg.CSnewname)) return;
...
void CDlgFormulaScintilla::OnDelete() {
  ...
  Formula()->FunctionCollection()->Delete(name);
  m_dirty = true;
  // Clear the tab
...
void CDlgFormulaScintilla::OnHandList()  {
  ...
  CString s = m_FormulaTree.GetItemText(m_FormulaTree.GetSelectedItem());
  COHScriptList *p_handlist = (COHScriptList*)Formula()->FunctionCollection()->LookUp(s);
...
void CDlgFormulaScintilla::OnBnClickedCalc() {
  ...
  // Processing for any other formula
    ...
    Formula()->FunctionCollection()->Dump();
    ret = Formula()->FunctionCollection()->Evaluate(m_current_edit);
    int line = Formula()->AutoplayerTrace()->GetLastEvaluatedRelativeLineNumber();
...
void CDlgFormulaScintilla::UpdateDebugAuto(void) {
  ...
  CString result = Formula()->FunctionCollection()->DebugTab()->EvaluateAll();
...
void CDlgFormulaScintilla::CopyTabContentsToFormulaSet() {
  for (int i=0; i<m_ScinArray.GetSize(); ++i) {
    ...
    Formula()->FunctionCollection()->SetFunctionText(name, 
